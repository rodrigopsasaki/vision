...
fastify.register(createMinimalVisionPlugin({
  performance: {
    trackExecutionTime: true,
...
setInterval(() => {
  ...
  if (memoryMB > 100) { // Alert if memory usage > 100MB
...
const start = async () => {
  try {
    ...
    console.log('🚀 Performance-optimized Fastify server with Vision is running on http://localhost:3002');
    console.log('');
