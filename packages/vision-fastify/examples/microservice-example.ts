...
async function callNotificationService(method: string, path: string, request: FastifyRequest, body?: any) {
  return callExternalService('notification-service', SERVICES.NOTIFICATION_SERVICE, method, path, request, body);
}

async function callAnalyticsService(method: string, path: string, request: FastifyRequest, body?: any) {
  return callExternalService('analytics-service', SERVICES.ANALYTICS_SERVICE, method, path, request, body);
}

async function callPaymentService(method: string, path: string, request: FastifyRequest, body?: any) {
  return callExternalService('payment-service', SERVICES.PAYMENT_SERVICE, method, path, request, body);
}
...
const start = async () => {
  try {
    ...
    console.log(`🚀 Microservice "${SERVICE_NAME}" v${SERVICE_VERSION} is running on http://localhost:${PORT}`);
    console.log('');
    console.log('🔗 Microservice endpoints:');
    console.log('  GET /api/users/123 - Get user with service enrichment');
    console.log('  POST /api/users - Create user (JSON: {"name": "Test", "email": "test@example.com"})');
