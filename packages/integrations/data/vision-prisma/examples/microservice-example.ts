...
const SERVICE_INSTANCE =
  process.env.HOSTNAME || `instance-${Math.random().toString(36).substr(2, 9)}`;
    ...
    },
    {
      ...
      success: (ctx) => {
        ...
        if (data["database.operation"]) {
          console.log(
            `METRIC: ${SERVICE_NAME}.database.operation.duration ${data["database.duration_ms"]}ms operation="${data["database.operation"]}"`,
          );
        }
      },
    },
