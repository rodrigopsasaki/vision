...
app.use(
  createVisionMiddleware({
    ...
    shouldExcludeRoute: (ctx) => {
      ...
      return (
        path.startsWith("/health") || path.startsWith("/metrics") || path.startsWith("/favicon")
      );
    },
  }),
);
...
app.use(async (ctx, next) => {
  ...
  if (requestCount % 100 === 0) {
    ...
    console.log(
      `📊 Performance stats: ${requestCount} requests, avg ${avgResponseTime}ms, memory ${memoryMB}MB`,
    );
...
app.listen(PORT, () => {
  console.log(
    `🚀 Performance-optimized Koa server with Vision is running on http://localhost:${PORT}`,
  );
  console.log("");
